...
import { createProductSchema, CreateProductFormValues } from "@/lib/schemas/product"
...
export function AddProductModal({ onClose, onSuccess, product }: AddProductModalProps) {
  ...
  const defaultCategoryId = useMemo(() => {
    ...
    if (typeof product.categoryId === 'object' && (product.categoryId as any)._id) {
       return (product.categoryId as any)._id
    }
          ...
          <div className="space-y-4">
             ...
             <div className="flex items-start gap-4 flex-wrap">
                ...
                {existingImages.map((img, idx) => (
                  <div key={`existing-${img._id}`} className="relative w-32 h-32 border rounded-lg overflow-hidden group">
                     <Image
                      width={128}
                      height={128}
                      src={img.url} 
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-[10px] px-1 py-0.5 text-center">Existing</div>
                  </div>
                ))}
                ...
                {newImagePreviews.map((src, idx) => (
                  <div key={`new-${idx}`} className="relative w-32 h-32 border rounded-lg overflow-hidden group">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-green-500/50 text-white text-[10px] px-1 py-0.5 text-center">New</div>
                  </div>
                ))}
             </div>

...
import { useReportTopBuyersSingle } from "@/lib/hooks/useRepost";
     ...
     if (isLoading) {
       return (
         <div className="flex flex-col items-center justify-center py-12 space-y-4">
           ...
           <p className="text-sm text-gray-500 animate-pulse">Loading buyer details...</p>
         </div>
       );
     }
         ...
         <div className="space-y-4">
           <div className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
             ...
             <div className="overflow-hidden">
               ...
               <p className="text-gray-900 font-medium truncate">{topBuyerData.email}</p>
             </div>
           </div>
           ...
           <div className="grid grid-cols-2 gap-4">
             <div className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
               ...
               <div>
                 ...
                 <p className="text-gray-900 font-bold text-lg">{topBuyerData.totalOrder}</p>
               </div>
             </div>
             <div className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
               ...
               <div>
                 ...
                 <p className="text-gray-900 font-bold text-lg">{topBuyerData.totalSpent}</p>
               </div>
             </div>
           </div>
         </div>
           ...
           <div className="border rounded-lg overflow-hidden">
             <table className="w-full text-xs text-left">
               <thead className="bg-gray-50 border-b">
                 <tr>
                   <th className="px-3 py-2 font-semibold text-gray-600">Order ID</th>
               ...
               <tbody className="divide-y divide-gray-100">
                 {topBuyerData.recentOrders && topBuyerData.recentOrders.length > 0 ? (
                   topBuyerData.recentOrders.map((order) => (
                     <tr key={order.orderUniqueId} className="hover:bg-gray-50">
                       <td className="px-3 py-3 font-medium text-gray-900">{order.orderUniqueId}</td>
                       <td className="px-3 py-3 text-gray-700">${order.totalPrice}</td>
                       <td className="px-3 py-3">
                         <div className="flex flex-col gap-1">
                          {order?.paymentStatus === 'paid' ? (
                            <span className={`px-2 py-0.5 rounded-full text-[10px] w-fit font-medium ${
